...
const fetchItems = async () => {
  ...
  try {
    const response = await masterDataService.fields.getAll();
    ...
    <Teleport to="body">
        <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
            <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl scale-100 animate-scale-in">
                <h3 class="text-xl font-bold text-white mb-6">{{ t('masterData.addField') }}</h3>
                <div class="space-y-4">
                    <div class="space-y-2">
                        ...
                        <input 
                            ...
                            :placeholder="t('masterData.addField')"
            ...
            <td class="table-cell text-right w-24 md:w-40 px-2 md:px-4 py-2">
              <div class="flex justify-end gap-2">
                <template v-if="editId !== item.id">
                    <button @click.stop="startEdit(item)" class="p-2 text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors" :title="t('common.edit')">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                        </svg>
                    </button>
                    <button @click.stop="handleDelete(item.id)" class="p-2 text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" :title="t('common.delete')">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
